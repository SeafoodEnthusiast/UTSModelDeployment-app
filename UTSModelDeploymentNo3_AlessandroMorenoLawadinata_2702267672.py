...
if not os.path.exists(DATA_PATH):
    st.error(f"Dataset file '{DATA_PATH}' tidak ditemukan.")
    st.stop()


predictor = HotelBookingPredictor(MODEL_PATH, DATA_PATH)

st.write("Silakan isi informasi pemesanan di bawah untuk prediksi:")
...
input_data = {
    ...
    'no_of_previous_bookings_not_canceled': st.number_input("Jumlah Booking Tidak Dibatalkan", 0, 20, 0),
...
if st.button("Gunakan Test Case 1 (Confirmed)"):
...
if st.button("Gunakan Test Case 2 (Canceled)"):
    st.session_state.update({
        'no_of_adults': 1,
        'no_of_children': 2,
